...
slate.Variants = (function() {
  ...
  Variants.prototype = Object.assign({}, Variants.prototype, {
    ...
    _onSelectChange: function(){
      console.log('thisss',this.product.variants);
     ...
     */
    _selectSwatch(){
      var _currentSplit = (this.currentVariant.title).indexOf('/') > -1 ? (this.currentVariant.title).split('/') : null;
      if( _currentSplit != null && _currentSplit.length == 2 ){
        
          console.log('[data-option-name="'+ this.product.options[1] +'"] .swatch-element[data-value="'+(_currentSplit[1]).trim()+'"]')
          $('[data-option-name="'+ this.product.options[0] +'"] .swatch-element[data-value="'+(_currentSplit[0]).trim()+'"]').find('input').prop("checked", true);
          $('[data-option-name="'+ this.product.options[1] +'"] .swatch-element[data-value="'+(_currentSplit[1]).trim()+'"]').find('input').prop("checked", true);
       
          // console.log('else_part');
          // $('[data-option-name="'+ this.product.options[0] +'"] .swatch-element[data-value="60 Serving Tub"]').find('input').prop("checked", true);
          // $('[data-option-name="'+ this.product.options[1] +'"] .swatch-element[data-value="Mango Pineapple"]').find('input').prop("checked", true);
        
      }
    },
    _hideSwatch: function(variant) {
      var _currentSplit = (this.currentVariant.title).indexOf('/') > -1 ? (this.currentVariant.title).split('/') : null;
      if( _currentSplit != null && _currentSplit.length == 2 ){
        var _fisrtOPtion = _currentSplit[0], _varaintArry = [],_cuVaraintArry = [];
        for( var i = 0; i < this.product.variants.length ; i++){ _varaintArry.push((this.product.variants[i].title).split('/')[1]); }
        function onlyUnique(value, index, self) { return self.indexOf(value) === index; }
        _varaintArry = _varaintArry.filter(onlyUnique);        
        for( var i = 0; i < _varaintArry.length ; i++){ _cuVaraintArry.push(_fisrtOPtion+"/"+_varaintArry[i]); }        
        for( var i = 0; i < _varaintArry.length ; i++){
          if($(this.originalSelectorId +' option[data-variat-title="'+ _cuVaraintArry[i]+'"]').length == 0 ){            
            $('[data-option-name="'+ this.product.options[1] +'"] .swatch-element[data-value="'+(_varaintArry[i]).trim()+'"]').addClass('hide');
          }else{ $('[data-option-name="'+ this.product.options[1] +'"] .swatch-element[data-value="'+(_varaintArry[i]).trim()+'"]').removeClass('hide');  }
        }
      }
    },
    _hideSize: function(variant){
      var _currentSplit = (this.currentVariant.title).indexOf('/') > -1 ? (this.currentVariant.title).split('/') : null;
      if( _currentSplit != null && _currentSplit.length == 2 ){
        var _fisrtOPtion = _currentSplit[1], _varaintArry = [],_cuVaraintArry = [];
        for( var i = 0; i < this.product.variants.length ; i++){ _varaintArry.push((this.product.variants[i].title).split('/')[0]); }
        function onlyUnique(value, index, self) { return self.indexOf(value) === index; }
        _varaintArry = _varaintArry.filter(onlyUnique);
        for( var i = 0; i < _varaintArry.length ; i++){ _cuVaraintArry.push(_varaintArry[i]+"/"+_fisrtOPtion); }        
        for( var i = 0; i < _varaintArry.length ; i++){          
          if($(this.originalSelectorId +' option[data-variat-title="'+ _cuVaraintArry[i]+'"]').length == 0 ){
            console.log('---',this.originalSelectorId +' option[data-variat-title="'+ _cuVaraintArry[i]+'"]')
            $('[data-option-name="'+ this.product.options[0] +'"] .swatch-element[data-value="'+(_varaintArry[i]).trim()+'"]').addClass('hide');
          }else{ $('[data-option-name="'+ this.product.options[0] +'"] .swatch-element[data-value="'+(_varaintArry[i]).trim()+'"]').removeClass('hide');  }
        }
      }
    },
...
theme.MobileNav = (function() {
  ...
  function openMobileNav() {
    ...
    var proListHeight = $(".wrap-mobile-nav-wrapper").find(".product-listing").outerHeight();
    var translateHeaderHeight = cache.$siteHeader.outerHeight() + proListHeight;

    console.log('proListHeight--',proListHeight);

    console.log("cache.$mobileNavContainer.prepareTransition()", cache.$mobileNavContainer.parent(".wrap-mobile-nav-wrapper").find(".product-listing").addClass("is_mNav_open"));
    cache.$mobileNavContainer.parent(".wrap-mobile-nav-wrapper").find(".product-listing").addClass("is_mNav_open");
    cache.$mobileNavContainer.parent(".wrap-mobile-nav-wrapper").addClass("nav_open_mobile");
    cache.$mobileNavContainer.prepareTransition().addClass(classes.navOpen);
	cache.$siteHeader.prepareTransition().addClass('site-'+classes.navOpen);
    ...
    $(window).on('scroll.site-header', function(evt) {
      if( translateHeaderHeight == cache.$siteHeader.outerHeight() ) return;
      // var proListHeight = $(".wrap-mobile-nav-wrapper").find(".product-listing").outerHeight();
    ...
    $('.inner-list').slick('refresh');
  }

  function closeMobileNav() {
    cache.$mobileNavContainer.prepareTransition().removeClass(classes.navOpen);
    cache.$siteHeader.prepareTransition().removeClass('site-'+classes.navOpen);

    cache.$mobileNavContainer.parent(".wrap-mobile-nav-wrapper").find(".product-listing").removeClass("is_mNav_open");
    cache.$mobileNavContainer.parent(".wrap-mobile-nav-wrapper").removeClass("nav_open_mobile");
    ...
    $(window).off('keyup.site-header');
    $(window).off('scroll.site-header');
...
theme.Slideshow = (function() {
  ...
  slideshow.prototype.setSlideshowHeight = function() {    
    var minAspectRatio = this.$slideshow.data('min-aspect-ratio');    
      this.$slideshow.height( window.innerWidth > 991 ? ($(document).width() / minAspectRatio) : ($(document).width() / minAspectRatio) + $('header.site-header').height());    
    var _this = this.$slideshow;
    setTimeout(function(){ if( window.innerWidth > 991 && ( $('header.site-header').offset().top + $('header.site-header').outerHeight()) >= _this.find('.slideshow__slide:first-child .slideshow__text-content').offset().top ){
      _this.height( ($(document).width() / minAspectRatio) + $('header.site-header').outerHeight() );
    } },500);
  };
...
theme.SearchDrawer = (function() {
  var selectors = {
    ...
    headerSectionContact:'.site-header-contact',
  ...
  function setHeight() {
    $(selectors.headerSection).find('.site-header').removeClass('site-header-scrolled');
    if($(selectors.headerSectionContact).length){
        $(selectors.drawer).css({
          height:  ($(selectors.headerSectionContact).outerHeight() + $(selectors.headerSection).find('.site-header').outerHeight() )
        });      
    }else{    
    $(selectors.drawer).css({
      height:  $(selectors.headerSection).outerHeight() > 1 ? $(selectors.headerSection).outerHeight() : $(selectors.headerSection).find('.site-header').outerHeight()
    });
      }
  }
      ...
      this._loadImage()
        .then(
          function(image) {
            ...
            image.style.width = ( image.width - ((image.width * 50)/100) )+ 'px';
            image.style.height = ( image.height - ((image.height * 50)/100) ) + 'px';
...
theme.Cart = (function() {
  ...
  var attributes = {
    ...
    cartItemSubProduct: 'data-subscribe-product',
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    ...
    _createCart: function(state) {
      ...
      var cart_total_amt = state.total_price / 100;
      var threshold_amt = Number($(".free-shipping-price").text());  
      ...
      if(result < 100){
        ...
        var success_message = $(".progress_result").attr("data-success");
    ...
    _createProductDetailsList: function(
      ...
      if (properties !== null && Object.keys(properties).length !== 0) {
        ...
        optionsPropertiesHTML = optionsPropertiesHTML.concat(
          this._getPropertySubscription(properties)
        );
      }
    ...
    _getPropertyList: function(properties) {
      ...
      return $.map(
        ...
        function(property) {
          ...
          if (property[0].charAt(0) === '_' || property[0] === 'subscription_id' || property[0] === 'shipping_interval_frequency' || property[0] === 'shipping_interval_unit_type') return;
     ...
     _getPropertySubscription: function(properties) {
      var propertiesArray =
        properties !== null ? Object.entries(properties) : [];
       var subscription = [];
       var $propertyElement = this.$itemPropertyTemplate.clone();
      $.map(
        propertiesArray,
        function(property) {          
          if (property[0] === 'shipping_interval_unit_type' || property[0] === 'shipping_interval_frequency'){            
            // if the property value has a length of 0 (empty), don't display it
            if (property[1].length === 0) return;
            subscription.push(property[1]);
          }
          
        }.bind(this)
      );
       if( subscription.length > 0){
         $propertyElement.find(selectors.cartItemPropertyName).text('Delivery frequency:');
         $propertyElement.find(selectors.cartItemPropertyValue).text(' Every ' + subscription[1]+' '+subscription[0]);
         $propertyElement.removeClass(classes.hide);
          return $propertyElement[0];
       }
    },
    ...
    _showRemoveMessage: function(lineItem) {      
      if(lineItem.attr(attributes.cartItemSubProduct) == "true"){ return ; }
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
      ...
      $.post(params)
        .done(
          function(item) {
            ...
            if(item.product_id == '6921931292837'){
    ...
    _getPropertyList: function(properties) {
      ...
      propertiesArray.forEach(function(property) {
        // Line item properties prefixed with an underscore are not to be displayed        
        if (property[0].charAt(0) === '_' || property[0] == 'shipping_interval_unit_type' || property[0] == 'shipping_interval_frequency') return;
    ...
    _initThumbnailSlider: function() {      
      ...
      if( $('.product-single__thumbnails').hasClass('slick-initialized') == false ){
            console.log("Check: product-single__thumbnails");
        $('.product-single__thumbnails').slick(options);
      }
      
      if( $('.big_slider').hasClass('slick-initialized') == false ){
        $('.big_slider').slick(options_main);
      ...
      if (variant) {
        if (variant.available) {
          ...
          $(this.selectors.addToCartText, this.$container)
            .html('<i class="fa fa-shopping-cart" aria-hidden="true"></i> '+theme.strings.addToCart)
            .text();
    ...
    _updatePrice: function(evt) {
      var variant = evt.variant;
      var variant_price = variant.price - (variant.price * 0.15);

      $('.product_save_price').html(theme.Currency.formatMoney(variant_price, theme.moneyFormat));
...
theme.Afaq = (function() {
  ...
  function Afaq(container) { }  
  Afaq.prototype = Object.assign({}, Afaq.prototype, {
    onBlockSelect: function(evt) {      
      if( !$(evt.target).hasClass(selectors.active)) {
        $(evt.target).find(selectors.faq_ans).trigger('click');
      }
    },
...
$(document).ready(function() {
  	$('.dark_border:first').addClass('border-botom');
  ...
  sections.register('faq-section', theme.Afaq);
    preloadLink();  
});

function preloadLink(){ var _link = document.getElementsByTagName('link'); for(var i = 0 ; i < _link.length ; i++ ){ if( _link[i].getAttribute('rel') == "preload" && _link[i].getAttribute('as') == "style"){ makeStyle(_link[i]); return false; } } }
function makeStyle(_link){ setTimeout(function(){ _link.rel='stylesheet'; preloadLink(); return false; },500); }

$(document).on('click', $.debounce( 100, function(e){
  if( $(e.target).closest('.btn--popup').length || $(e.target).hasClass('.btn--popup').length || $(e.target).closest('.modal__close').length || $(e.target).hasClass('.modal__close').length ){ return; }
  if($('.block_img--popup').hasClass('modal--is-active')){
    if( $(e.target).closest('.modal__centered').length ){}else{
      $('body').find('.modal.modal--is-active .modal__close').trigger('click');
    }
  }
}) );

$(document).on('click','.plus', $.debounce( 10, function(e){
  var $qty = $(this).closest('.QuantityBox').find(".QuantityInput");
  var currentVal = parseInt($qty.val(), 10);
  if (!isNaN(currentVal)) {
    $qty.val(currentVal + 1).change();
  }
}) );
$(document).on('click','.minus', $.debounce( 10, function(e){
  var $qty = $(this).closest('.QuantityBox').find(".QuantityInput");
  var currentVal = parseInt($qty.val(), 10);
  var min_value = $(this).closest('.QuantityBox').find(".QuantityInput").attr('min') ? Number( $(this).closest('.QuantityBox').find(".QuantityInput").attr('min') ) : 1;  
  if (!isNaN(currentVal) && currentVal > min_value) {
    $qty.val(currentVal - 1).change();
  }
}) );
...
$(document).on('click','.btn--popup',function(event){
  if($($(this).attr('href')).length){
    ...
    var _overflow = $(this).attr('data-popup-overflow') != undefined ? $(this).attr('data-popup-overflow') : 'false';
    var _popup = $($(this).attr('href'));
    if( _popup.closest('#PageContainer').length ){ _popup.closest('#PageContainer').addClass('transform--none');
                                                 _popup.addClass('modal--is-active');                                                  
                                                 }else{ _popup.addClass('modal--is-active'); }
    _overflow == 'true' ? $('body,html').addClass('btn-popup--overflow') : null;
    
  }
});
$(document).on('click','.modal__close',function(event){
  	$(this).closest('.modal--is-active').removeClass('modal--is-active');
  	if( $(this).closest('#PageContainer').length ){ $(this).closest('#PageContainer').removeClass('transform--none'); }
  if( $('body,html').hasClass('btn-popup--overflow') ){ $('body,html').removeClass('btn-popup--overflow'); }
});

$('a[href^="#"]').click(function(event) {  
  var _header_height = 25, _id = $(this).attr('href');   
  if( $(_id).length ) {
    event.preventDefault();
    $('html, body').animate({
      scrollTop: $(_id).offset().top - _header_height
    }, 100);
  }
});

_accordian('.pd-header_ac',768);
_accordian('.b_sb--title',768);
_accordian('.refe_title');
_accordian('.tab_content_title',768,true,true);

function _accordian(name,startAfter,parentHasActiveClass = false, sclose = false){
  if( startAfter == undefined )
  {
    $(document).on('click', name ,function(){
      _accordianInner($(this),parentHasActiveClass,sclose);      
    });
  }else{
    $(document).on('click', name ,function(){
      if(window.innerWidth < startAfter){        
        if(sclose){ 
          $(this).parent().siblings().find(name).next().slideUp();
          $(this).parent().siblings().removeClass('active');
          $(this).parent().siblings().find(name).removeClass('active');
        }        
        _accordianInner($(this),parentHasActiveClass,sclose);
      }
    });
...
function _accordianInner(_this,parentHasActiveClass,sclose){
  var _class = _this.attr('class');
  _class = _class.replace('active','');  
  if(!_this.hasClass('active')){
    _this.next().slideDown();
    _this.addClass('active');    
    parentHasActiveClass ? _this.parent().addClass('active') : null ;
  }else{ _this.next().slideUp();
        _this.removeClass('active');
       	parentHasActiveClass ? _this.parent().removeClass('active') : null ;
       }    
  return false;
}
theme.init = function() {
...
window.onload = $.debounce(10, function() { bgImageSizeHeight(); fixedHeader(); });
window.addEventListener('resize', $.debounce(100, function() {
  if( document.getElementsByClassName('site-header-scrolled')[0] != undefined ) { document.getElementsByClassName('site-header-scrolled')[0].classList.remove('site-header-scrolled');}
  fixedHeader();
  if ( document.getElementsByClassName('temp-header')[0] != undefined){
    if(document.body.scrollTop > document.getElementsByClassName('temp-header')[0].offsetHeight || document.documentElement.scrollTop > document.getElementsByClassName('temp-header')[0].offsetHeight ) {
    document.getElementsByClassName('site-header-fixed')[0].classList.add('site-header-scrolled');
      checkHeaderOffset();
    }
  }
  bgImageSizeHeight();
...
window.onscroll = $.debounce(10, function() {  
  var _header = document.getElementsByClassName('h_s_home')[0] != undefined ? document.getElementsByClassName('h_s_home')[0] : document.getElementsByClassName('site-header-statick')[0];
  if( _header ){    
    if (document.body.scrollTop > _header.offsetHeight || document.documentElement.scrollTop > _header.offsetHeight ) {
      _header.classList.add('site-header-scrolled');
      if( document.getElementsByClassName('site-header-statick')[0] ) { checkHeaderOffset();  }
    } else {
     _header.classList.remove('site-header-scrolled');
      if( document.getElementsByClassName('site-header-statick')[0] ){
        checkHeaderOffset();
      }
    }
  }
...
function checkHeaderOffset(){
  var _header = document.getElementsByClassName('site-header-statick')[0], _headerParet = _header.parentNode;
  if(_headerParet.querySelector('.site-header-contact')){
    // _header.style.top = _headerParet.querySelector('.site-header-contact').offsetHeight+"px";
  }
  if( _header.classList.contains('site-header-scrolled')){ _header.style.top = 0; }
  return true;
}
function fixedHeader(){
  var temp_header = document.getElementsByClassName('temp-header')[0], 
      site_header = document.getElementsByClassName('site-header-statick')[0],
      scrolled = false;

  if( document.getElementsByClassName('temp-header').length){    
    for( var i = 0 ; i < site_header.classList.length ; i++){
      if(site_header.classList[i] == 'site-header-scrolled'){
        scrolled = true;
      }
    }    
    if ( !scrolled ){ temp_header.style.height = site_header.offsetHeight + "px"; }
    site_header.classList.add("site-header-fixed");
    checkHeaderOffset();
    if(scrolled && temp_header.offsetHeight == 0 ){
      if( site_header.classList.contains('site-header-scrolled') ){ site_header.classList.remove('site-header-scrolled'); } 
      temp_header.style.height = site_header.offsetHeight + "px";
      if( site_header.classList.contains('site-header-scrolled') ){ site_header.classList.add('site-header-scrolled'); }
    }
  }
  return true;
}
function bgImageSizeHeight(){
  $('.gs_item.gs_item-two').find('.gs_img__grid').css('padding-bottom','');
  $('.gs_item').find('.gs-bg-img').css('padding-bottom','').removeClass('custom-padding-js');
  $('.gs_item.gs_item-two').find('img[data-width]').css('max-width','');
  $('.gs_item').each(function(){
    var _array = [];
    var _this = $(this);
    $(this).parent().find('.gs_wrapper h2').css('min-height','');
    $(this).parent().find('.gs_wrapper h2').each(function(index){
      _array.push($(this).outerHeight());
    });    
    function onlyUnique(value, index, self) {  return self.indexOf(value) === index; }
    var unique = _array.filter( onlyUnique );    
    if( unique.length > 1 && $(this).parent().find('.gs_wrapper h2').length > 1 ){
      $(this).parent().find('.gs_wrapper h2').css('min-height',(Math.max.apply(null, _array))+'px')
    }

    var _inner_height = ($(this).find('.gs_wrapper .div_top').outerHeight() + $(this).find('.gs_wrapper .div_bottom').outerHeight());
    if( _inner_height >= $(this).find('.gs_wrapper').height() ){
      var _offset_spaing = ( $(this).offset().top + $(this).height()) - ( $(this).find('.gs_wrapper').offset().top + $(this).find('.gs_wrapper').height() );
      _offset_spaing += _inner_height;
      if( $(this).hasClass('gs_item-two') ){
        $(this).find('.gs_img__grid').css('padding-bottom', _offset_spaing+'px');        
        if( $(this).parent().find('.type_block').length > 0 ){
          window.innerWidth < 768 ? $(this).find('.gs_img__grid').css('padding-bottom', _offset_spaing+'px') : $(this).parent().find('.type_block .gs_img__grid').css('padding-bottom', _offset_spaing+'px');
        }
      }else{
        window.innerWidth < 768 ? $(this).find('.gs-bg-img').css('padding-bottom', _offset_spaing+'px').addClass('custom-padding-js') : $(this).parent().find('.gs-bg-img').css('padding-bottom', _offset_spaing+'px').addClass('custom-padding-js');
      }
    }

    if( $(this).find('img[data-width]').length && window.innerWidth < 1680 ){      
      var _default_width =  $(this).width() / 960;      
      $(this).find('img[data-width]').css('max-width', ($(this).find('img[data-width]').attr('data-width') *  _default_width ) +'px')
    }
  });
  return true;
...
$(document).on('click', '.acc_ttl', function(){
   $(this).toggleClass('active').parents('.faq_items').siblings().find('.acc_ttl').removeClass('active');
   $(this).parents('.faq_items').find('.body_faq').slideToggle().parents('.faq_items').siblings().find('.body_faq').slideUp();
 });
...
$(document).on('click', '.icon-hamburger', function(){
   $(this).parents('body').addClass('open_nav');
 });  
$(document).on('click', '.icon-close', function(){
   $(this).parents('body').removeClass('open_nav');
 });   
$(document).mouseup(function(e){
  var popup = $(".cart-side-panel");
  if(!$('#cart.shopping-cart').is(e.target) && !popup.is(e.target) && popup.has(e.target).length == 0){
  ...
  //   var $qty = $(this).parents('.wc_qty_box').find(".QuantityBox .QuantityInput");
  //   var currentVal = parseInt($qty.val(), 10);
  //   if (!isNaN(currentVal)) {
  ...
  // $(document).on('click','.pdp_tabinner.faq-li',function(){
  //   $(this).find('.faq-ans').toggle();
  // });
...
$('.product-review-wrapper').slick({
  centerMode: true,
  centerPadding: '50px',
  slidesToShow: 1,
  dots: true,
  prevArrow: '<button type="button" class="slick-prev slick-arrow"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="60" viewBox="0 0 32 60" fill="none"><g clip-path="url(#clip0_128_10377)"><path opacity="0.502" fill-rule="evenodd" clip-rule="evenodd" d="M31.9969 54.856L10.4379 30.006L31.9969 5.136L26.0199 0.00299835L0.00292969 30.006L26.0199 59.998L31.9969 54.856Z" fill="#384151"/></g><defs><clipPath id="clip0_128_10377"><rect width="32" height="60" fill="white"/></clipPath></defs></svg></button>',
  nextArrow: '<button type="button" class="slick-next slick-arrow"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="60" viewBox="0 0 32 60" fill="none"><g clip-path="url(#clip0_128_10379)"><path opacity="0.502" fill-rule="evenodd" clip-rule="evenodd" d="M0.00292969 54.856L21.5619 30.006L0.00292969 5.136L5.97993 0.00299835L31.9969 30.006L5.97993 59.998L0.00292969 54.856Z" fill="#384151"/></g><defs><clipPath id="clip0_128_10379"><rect width="32" height="60" fill="white"/></clipPath></defs></svg></button>',
  responsive: [
  {
    breakpoint: 991,
    settings: {
...
// $('.custom_product_add_wrapper').click(function(e){
//     e.preventDefault();
//     var formData = $(this).data('variant-id');
//     var product_qty = $(this).data('qty'); 
//     addItemToCart(formData,product_qty);    
// });
// function addItemToCart(variant_id,variant_qty) {
//     data = {
//       "id": variant_id,
...
$(".swatch-element input").on("click", function() {
   ...
   var correspondingOption = $(".single-option-selector option[value='" + radioValue + "']");
   correspondingOption.prop("selected", true).trigger("change");
 });
 ...
 $('.page-new-static-page .product-form__cart-submit').on('click', function(e) {
     e.preventDefault();
      var variantId = Number($('.product-form__variants option').val());
       var data = {
        "id": variantId,
        "quantity": 1 
      };
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          console.log('Product added to the cart:', response);
          window.location.href = '/checkout'; 
        },
      });
    });

$('.page-new-static-nitric-oxide .product-form__cart-submit').on('click', function(e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(response) {
            var data = {
                ...
                "quantity": 1,
                "properties": {
                    "Note": "Nitric Oxide Test Strips (Free)"
                }
            };
            console.log(variantId);
            $.ajax({
                ...
                success: function(response) {
                    $.ajax({
                        ...
                        type: 'POST',
                        dataType: 'json',
                        data: { note: "Nitric Oxide Test Strips (Free)" },
                        success: function(result) {
                            window.location.href = '/checkout?discount=NO15'; 
                        },
                        error: function(jqxhr, status, exception) {
                            console.log('Error updating cart note:', exception);
                        }
                    });
                },
                error: function(jqxhr, status, exception) {
                    console.error('Error adding product to cart:', exception);
                }
            });
        },
        error: function(jqxhr, status, exception) {
            console.error('Error clearing cart:', exception);
        }
    });
});


$('.page-new_prebiotic_product .product-form__cart-submit').on('click', function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.ajax({
                ...
                data: data,
                success: function(response) {
               window.location.href = '/checkout?discount=PRE15'; 
                },
                error: function(jqxhr, status, exception) {
                    console.error('Error adding product to cart:', exception);
                }
...
// new magnesium product 

$('.page-new-magnesium .product-form__cart-submit').on('click', function(e) {
...
// end update new magnesium ******************


$('.page-nitric-oxide-20-percent-off .product-form__cart-submit').on('click', function(e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.ajax({
                ...
                data: data,
                success: function(response) {
                 window.location.href = '/checkout?discount=NITRIC20'; 
                },
                error: function(jqxhr, status, exception) {
                    console.error('Error adding product to cart:', exception);
                }
    ...
    $('.nutrition').on('click', function() {
        $('.ingredient_modal, .ingre_table_overlay').toggle();
    });
    $('.ingre_table_overlay').on('click', function() {
        $(this).css("display","none");
        $('.ingredient_modal').css("display","none");
    });
$(document).ready(function(){
  if ($('body').hasClass('page-new-static-page')) {
     setTimeout(function(){
...
$('.trigger_click_btn').click(function(event){
    event.preventDefault()
    $('.product-form__cart-submit').trigger('click')
})

if (window.outerWidth < 575) {
var spanElement = document.querySelector('.product-sleep-product-layout .swatch_30-serving-tub span.swatch_label'); 
if (spanElement) {
  spanElement.innerHTML = spanElement.innerHTML.replace('Tub', '');    
}
}
